...
func main() {
	...
	config.AddServer("localhost", "user1", "password1", []string{"nginx", "prometheus"}, "localhost")
	config.AddServer("10.1.0.170", "user2", "password2", []string{"fluentd", "nginx"}, "server")
	config.AddServer("10.1.0.172", "user3", "password3", []string{}, "client") // 빈 프로세스 목록은 기본값 사용
